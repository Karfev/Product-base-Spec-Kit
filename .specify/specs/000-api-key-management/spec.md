...
Добавляем API-ключи — долгосрочные credentials для программного доступа к платформе.
Пользователи смогут создавать, просматривать и отзывать ключи через REST API.
Секрет ключа возвращается единственный раз при создании и нигде не хранится в открытом виде.

## Motivation / Problem

Enterprise-клиенты не могут автоматизировать интеграции без постоянных credentials.
Bearer token-сессии истекают, вынуждая сервисные аккаунты регулярно переаутентифицироваться — это неприемлемо для unattended-процессов.
Детали в PRD: `../../../initiatives/INIT-2026-000-api-key-management/prd.md`
